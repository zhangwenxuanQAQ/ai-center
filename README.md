...
6. **提示词管理**：提示词模板的创建、编辑和管理
7. **数据源管理**：支持MySQL、PostgreSQL、Oracle、SQL Server、RustFS、S3等多种数据源
8. **用户管理**：用户权限和角色管理
9. **聊天记录管理**：完整的对话历史记录和检索功能，支持聊天重连（切换对话或刷新页面后可继续回答）
10. **系统监控**：实时监控系统运行状态和性能指标

### 技术特性

- **RAG增强检索**：集成先进的文档解析和向量检索技术，支持DeepDoc文档解析；知识库可转化为工具供大模型调用（react agent模式）
- **内置工具调用**：支持大模型在对话中自动调用内置工具（如网络搜索），通过 react agent 实现任务规划和工具执行
- **富文本渲染**：聊天界面支持 markdown-ui 交互和 mermaid 图表渲染，统一的 ChatMarkdown 组件支持代码块展开收起
- **多模型支持**：支持文本生成、图像理解、语音识别、语音合成等多种AI能力；支持深度思考开关及参数配置
- **企业微信集成**：支持与企业微信对接，实现智能客服功能
- **机器人插件集成**：支持第三方插件嵌入，插件聊天支持断点续传、文件上传下载、历史会话保存
...
### 6. 内置工具与 React Agent
- 大模型支持内置工具调用，采用 OpenAI Tool 参数规范
- 内置网络搜索工具（web_search），可在配置文件中配置搜索引擎和 API Key
- 支持 react agent 模式，大模型可自动规划和执行任务，将知识库作为工具调用
- 前端参数（如网络搜索开关、深度思考开关）在后端传递给模型前会自动过滤，避免参数不兼容
...
web_search_engine:
  enabled: true          # 是否启用网络搜索功能，默认 true
  host: '127.0.0.1'      # SearXNG 搜索引擎服务地址
  port: 8080             # SearXNG 搜索引擎服务端口
  engines: 'baidu'       # 默认使用的搜索引擎，支持 baidu、bing、google 等

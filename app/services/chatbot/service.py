...
class ChatbotService:
    ...
    def _get_or_create_default_category():
        ...
        """
        default_category = (
            ChatbotCategory.select().where(ChatbotCategory.is_default == True).first()
        )
        if not default_category:
            default_category = ChatbotCategory(
                name="默认分类", description="系统默认分类", is_default=True
            )
    ...
    def get_source_configs_with_callback():
        ...
        for source_type in SOURCE_TYPE.keys():
            ...
            if source_type == "work_weixin":
                ...
                callback_url = (
                    f"http://{server_host}:{server_port}/aicenter/v1/chat/work_weixin"
                )
    ...
    def create_chatbot(chatbot: ChatbotCreate):
        ...
        try:
            ...
            if code:
                existing_chatbot = (
                    Chatbot.select()
                    .where((Chatbot.code == code) & (Chatbot.deleted == False))
                    .first()
                )
                if existing_chatbot:
            ...
            if mcp_server_ids:
                for mcp_server_id in mcp_server_ids:
                    chatbot_mcp = ChatbotMCP(
                        chatbot_id=db_chatbot.id, mcp_server_id=mcp_server_id
                    )
    ...
    def get_chatbots(
        ...
        try:
            ...
            for chatbot in chatbots:
                # 获取MCP服务关联
                mcp_server_ids = [
                    str(cm.mcp_server_id)
                    for cm in ChatbotMCP.select().where(
                        ChatbotMCP.chatbot_id == chatbot.id
                    )
                ]
                # 构建聊天机器人字典
                chatbot_dict = {
                    ...
                    "category_id": (
                        str(chatbot.category_id).replace("-", "")
                        if chatbot.category_id
                        else None
                    ),
    ...
    def get_chatbot(chatbot_id: str):
        ...
        mcp_server_ids = [
            str(cm.mcp_server_id)
            for cm in ChatbotMCP.select().where(ChatbotMCP.chatbot_id == chatbot.id)
        ]
        chatbot_dict = {
            "id": str(chatbot.id),
            "code": chatbot.code,
    ...
    def update_chatbot(chatbot_id: str, chatbot: ChatbotUpdate):
        ...
        try:
            ...
            try:
                ...
                raise ResourceNotFoundError(message=f"聊天机器人 {chatbot_id} 不存在")
            ...
            if mcp_server_ids is not None:
                ...
                for mcp_server_id in mcp_server_ids:
                    chatbot_mcp = ChatbotMCP(
                        chatbot_id=chatbot_id, mcp_server_id=mcp_server_id
                    )
                    chatbot_mcp.save(force_insert=True)
                ...
                try:
                    ...
                    if not llm_model.deleted and llm_model.status:
                        model_info = {
                            ...
                            "tags": (
                                json.loads(llm_model.tags) if llm_model.tags else []
                            ),
                            "avatar": (
                                llm_model.avatar
                                if hasattr(llm_model, "avatar")
                                else None
                            ),
                            "binding_id": str(chatbot_model.id),
                            "config": (
                                json.loads(chatbot_model.config)
    ...
    def get_chatbot_model_by_type(chatbot_id: str, model_type: str):
        ...
        try:
            ...
            model_info = {
                ...
                "binding_id": str(chatbot_model.id),
                "config": (
                    json.loads(chatbot_model.config) if chatbot_model.config else {}
                ),
                "created_at": (
                    llm_model.created_at.strftime("%Y-%m-%d %H:%M:%S")
                    if llm_model.created_at
                    else None
                ),
                "updated_at": (
                    llm_model.updated_at.strftime("%Y-%m-%d %H:%M:%S")
                    if llm_model.updated_at
                    else None
                ),
            }
    ...
    @staticmethod
    @handle_transaction
    def bind_model_to_chatbot(
        chatbot_id: str, model_id: str, model_type: str, config: dict = None
    ):
        """
        绑定模型到机器人
        ...
        try:
            ...
            if llm_model.model_type != model_type:
                raise Exception(
                    f"模型类型不匹配，期望类型：{model_type}，实际类型：{llm_model.model_type}"
                )
        ...
        try:
            ...
            if not chatbot_model:
                raise ResourceNotFoundError(
                    message=f"机器人 {chatbot_id} 未绑定类型为 {model_type} 的模型"
                )
        ...
        try:
            ...
            for chatbot_prompt in query:
                prompt_info = {
                    ...
                    "created_at": (
                        chatbot_prompt.created_at.strftime("%Y-%m-%d %H:%M:%S")
                        if chatbot_prompt.created_at
                        else None
                    ),
                }
                            ...
                            prompt_info.update(
                                {
                                    ...
                                    "tags": (
                                        json.loads(prompt.tags) if prompt.tags else []
                                    ),
                                }
                            )
    ...
    def bind_prompt_to_chatbot(
        ...
        try:
            ...
            if prompt_source == "library":
                ...
                try:
                    ...
                    if prompt.deleted or not prompt.status:
                        raise ResourceNotFoundError(
                            message=f"提示词 {prompt_id} 不存在或未启用"
                        )
                except Prompt.DoesNotExist:
            ...
            if sort_order <= 0:
                ...
                max_sort_order = (
                    ChatbotPrompt.select(fn.MAX(ChatbotPrompt.sort_order))
                    .where(
                        (ChatbotPrompt.chatbot_id == chatbot_id)
            ...
            if prompt_binding_id:
                try:
                    ...
                    chatbot_prompt.prompt_id = (
                        prompt_id if prompt_source == "library" else None
                    )
                    chatbot_prompt.prompt_name = (
                        prompt_name if prompt_source == "manual" else None
                    )
                    chatbot_prompt.prompt_content = (
                        prompt_content if prompt_source == "manual" else None
                    )
                    ...
                    raise ResourceNotFoundError(
                        message=f"绑定关系 {prompt_binding_id} 不存在"
                    )
            else:
    ...
    def unbind_prompt_from_chatbot(chatbot_id: str, prompt_binding_id: str):
        ...
        try:
            ...
            if not chatbot_prompt:
                raise ResourceNotFoundError(
                    message=f"提示词绑定关系 {prompt_binding_id} 不存在"
                )

            ChatbotPrompt.delete().where(
                ChatbotPrompt.id == chatbot_prompt.id
            ).execute()
    ...
    @staticmethod
    @handle_transaction
    def update_prompt_sort_order(
        chatbot_id: str, prompt_binding_id: str, sort_order: int
    ):
        """
        更新提示词排序
    ...
    def get_chatbot_tools(chatbot_id: str):
        ...
        try:
            chatbot_tools = ChatbotTool.select().where(
                (ChatbotTool.chatbot_id == chatbot_id) & (ChatbotTool.deleted == False)
            )
    ...
    def _enrich_tool_binding(tool_type: str, configs: dict) -> dict:
        ...
        try:
            if tool_type == "mcp":
                ...
                server_id = configs.get("server_id", "")
                api_ids = (
                    configs.get("api_ids", []) or configs.get("tool_ids", []) or []
                )
                ...
                if tool_type == "code_script":
                    # 代码脚本：查库回填脚本名称/描述
                    from app.database.models import CodeScript

                    script_ids = (
                        configs.get("tool_ids", [])
                        or configs.get("tool_names", [])
                        or []
                    )
                    item["server_name"] = "代码脚本"
                    if script_ids:
                        scripts = list(
                            ...
                            item["tools"].append(
                                {
                                    ...
                                    "tool_title": s.name,
                                    "tool_name": s.name,
                                    "tool_description": s.description or "",
                                    "tool_type": "code_script",
                                }
                            )
                else:
                    # skill：暂仅回填configs中的名称信息
                    ids = configs.get("tool_ids", []) or []
                    names = configs.get("tool_names", []) or []
                    item["server_name"] = "skill"
    ...
    def bind_tool_to_chatbot(chatbot_id: str, tool_type: str, configs: dict):
        ...
        try:
            ...
            if existing:
                # 合并工具ID列表
                merged = ChatbotService._merge_configs(
                    tool_type, existing_configs, configs
                )
    ...
    def unbind_tool_from_chatbot(chatbot_id: str, tool_binding_id: str):
        ...
        try:
            ...
            if not chatbot_tool:
                raise ResourceNotFoundError(
                    message=f"工具绑定关系 {tool_binding_id} 不存在"
                )
    ...
    def bind_knowledgebase_to_chatbot(chatbot_id: str, knowledgebase_id: str):
        ...
        try:
            ...
            chatbot_kb = ChatbotKnowledgebase(
                chatbot_id=chatbot_id, knowledgebase_id=knowledgebase_id
            )
    ...
    def unbind_knowledgebase_from_chatbot(chatbot_id: str, kb_binding_id: str):
        ...
        try:
            ...
            if not chatbot_kb:
                raise ResourceNotFoundError(
                    message=f"知识库绑定关系 {kb_binding_id} 不存在"
                )

            ChatbotKnowledgebase.delete().where(
                ChatbotKnowledgebase.id == chatbot_kb.id
            ).execute()

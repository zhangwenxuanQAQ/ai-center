...
class GenerateParam(ComponentParamBase):
    ...
    def __init__(self):
        ...
        self.only_use_user_message = False  # messages是否只使用user消息
        self.stream = True  # 当下游是answer是默认流式输出
        self.messages = []  # 问答记录
    ...
    def gen_conf(self):
        ...
        if self.deep_thinking is not None:
            conf["deep_thinking"] = self.deep_thinking
        return conf
...
class Generate(ComponentBase):
    ...
    def get_dependent_components(self):
        inputs = self.get_input_elements()  # 组件输入依赖
        prompt_inputs = self.get_prompt_input_elements()  # 提示词输入依赖
    ...
    def get_prompt_input_elements(self):
        ...
        key_set = set([])
        res = []
        for r in re.finditer(r"\{([a-z0-9]+[:@.][a-z0-9_-]+)\}", self._param.prompt, flags=re.IGNORECASE):
            cpn_id = r.group(1)
    ...
    def _run(self, history, **kwargs):
        ...
        for para in self.get_prompt_input_elements():
            ...
            if para["key"].startswith("sys."):
                ...
                if global_value:
                    if "sys.files" == para["key"]:
                        file_content = "----\n".join(global_value)
                        kwargs[para["key"]] = file_content
                    else:
                        kwargs[para["key"]] = global_value
                    self._param.inputs.append(
                        {"component_id": para["key"], "content": kwargs[para["key"]]})
                continue
        ...
        if '历史问题：""""""' in prompt:
            ...
            for item in history[:-1]:
                if item[0] == 'user':
                    history_query.append(item[1])
            history_query = '\n'.join(history_query)
            prompt = prompt.replace('历史问题：""""""', f"历史问题：\"{history_query}\"")
        ...
        if chat_mdl.max_length < 18000:
            chat_mdl.max_length = 18000

        if self._param.only_use_user_message:  # 只使用user输入
            msg = [x for x in msg if x["role"] == "user" or x["role"] == "system"]

        _, msg = message_fit_in([{"role": "system", "content": prompt}, *msg], int(chat_mdl.max_length * 0.97))
        # if len(msg) < 2:
        #     msg.append({"role": "user", "content": "Output: "})

        # msg[-1]["content"] = query  # 设置本次llm用户输入为query
    ...
    def get_messages(self, window_size):
        convs = []
        if window_size <= 0:
            return convs
        msg = self._param.messages
        messages = []
        for msg in msg:
            if msg["role"] != "system":
                messages.append(msg)

        return messages[window_size * -1:]

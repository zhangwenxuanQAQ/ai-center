                ...
                if item["cpn_id"].find("@") > 0:
                    ...
                    for p in self._canvas.get_component(cid)["obj"]._param.query:
                        if p["key"] == key:
                            ...
                            break
                elif item["cpn_id"].startswith("sys."):
                    global_value = self._canvas.get_global_value(item["cpn_id"])
                    if global_value:
                        res_content = ""
                        if "sys.files" == item["cpn_id"]:
                            file_content = "----\n".join(global_value)
                            res_content = file_content
                        else:
                            res_content = global_value

                        res.append(self.process_operator(res_content, item["operator"], item.get("value", "")))
                else:
    ...
    def process_operator(self, input: str, operator: str, value: str) -> bool:
        ...
        if operator == "contains":
            ...
            try:
                ...
                return True if input <= value else False
        elif operator == "in" or operator == "not in":  # 存在于/不存在于
            try:
                memo_result = get_memory_result_by_config_ids([value], json.loads(str(self._canvas)))
                if memo_result:
                    value = memo_result[0]["value"]
                if not value:
                    return False
                if operator == "in":
                    return True if input in value else False
                if operator == "not in":
                    return True if input not in value else False
            except Exception:
                if operator == "in":
                    return True if input in value else False
                if operator == "not in":

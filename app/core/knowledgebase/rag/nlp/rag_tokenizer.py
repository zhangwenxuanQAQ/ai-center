...
# 设置NLTK数据路径
nltk_data_path = os.path.join(get_project_base_directory(), "app/core/knowledgebase/rag/res", "nltk_data")
...
class RagTokenizer:
    ...
    def _load_dict(self, fnm):
        ...
        try:
            ...
            import pickle
            with open(dict_file_cache, 'wb') as f:
                pickle.dump(self.trie_, f)
            of.close()
        except Exception:
    ...
    def __init__(self, debug=False):
        ...
        self.DIR_ = os.path.join(get_project_base_directory(), "app/core/knowledgebase/rag/res", "huqie")
    ...
    def dfs_(self, chars, s, preTks, tkslist, _depth=0, _memo=None):
        ...
        if s + 2 <= len(chars):
            t1 = "".join(chars[s:s + 1])
            t2 = "".join(chars[s:s + 2])
            if self.key_(t1) in self.trie_ or self.trie_.has_subtrie(self.key_(t1)) and not (self.key_(t2) in self.trie_ or self.trie_.has_subtrie(self.key_(t2))):
                S = s + 2
        if len(preTks) > 2 and len(preTks[-1][0]) == 1 and len(preTks[-2][0]) == 1 and len(preTks[-3][0]) == 1:
            t1 = preTks[-1][0] + "".join(chars[s:s + 1])
            if self.key_(t1) in self.trie_ or self.trie_.has_subtrie(self.key_(t1)):
                S = s + 2

        for e in range(S, len(chars) + 1):
            t = "".join(chars[s:e])
            k = self.key_(t)
            if e > s + 1 and not (k in self.trie_ or self.trie_.has_subtrie(k)):
                break
            if k in self.trie_:
    ...
    def _max_forward(self, line):
        ...
        while s < len(line):
            e = s + 1
            t = line[s:e]
            while e < len(line) and (self.key_(t) in self.trie_ or self.trie_.has_subtrie(self.key_(t))):
                e += 1
                t = line[s:e]
    ...
    def _max_backward(self, line):
        ...
        while s >= 0:
            e = s + 1
            t = line[s:e]
            while s > 0 and (self.rkey_(t) in self.trie_ or self.trie_.has_subtrie(self.rkey_(t))):
                s -= 1
                t = line[s:e]
    ...
    def is_chinese(self, text: str) -> bool:
        ...
        if not text:
            return False
        chinese_count = sum(1 for ch in text if is_chinese(ch))

...
参考ragflow的term_weight实现，用于计算token的权重
"""
...
class Dealer:
    ...
    def __init__(self):
        ...
        fnm = os.path.join(get_project_base_directory(), "app/core/knowledgebase/rag/res")
        self.ne, self.df = {}, {}
        try:
            self.ne = json.load(open(os.path.join(fnm, "ner.json"), "r", encoding='utf-8'))
        except Exception as e:
            logger.warning(f"Load ner.json FAIL: {e}")
        try:
            self.df = load_dict(os.path.join(fnm, "term.freq"))
        except Exception as e:
            logger.warning(f"Load term.freq FAIL: {e}")
    ...
    def weights(self, tks, preprocess=True) -> List[Tuple[str, float]]:
        ...
        Returns:
            List[Tuple[str, float]]: (token, weight)列表
        """

...
from app.core.knowledgebase.rag.prompts.template import load_prompt
from app.constants.llm_constants import TAG_FLD
...
async def content_tagging(chat_mdl, content, all_tags, examples, topn=3):
    ...
    for ex in examples:
        ex["tags_json"] = json.dumps(ex.get(TAG_FLD, []), indent=2, ensure_ascii=False)
